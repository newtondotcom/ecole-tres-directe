...
const apiVersion = import.meta.env.VITE_ECOLE_DIRECTE_API_VERSION ?? "7.0.1";
...
function RouteComponent() {
  ...
  const handleValidateSession = async () => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : "Impossible de valider la session.";
      setError(message);
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      function handleGtkMessage(event: MessageEvent) {
        ...
        if (message?.action === "gtkRulesUpdated") {
          ...
          cleanup();
          reject(new Error("Impossible de récupérer le cookie GTK."));
        }
      }

      window.addEventListener("message", handleGtkMessage);
      fetch(
        `https://api.ecoledirecte.com/v3/login.awp?gtk=1&v=${encodeURIComponent(apiVersion)}`,
      ).catch(() => {
        cleanup();
        reject(new Error("La requête vers EcoleDirecte a échoué. Vérifiez votre connexion."));
      });
    });
  }, [extensionDetected]);
                ...
                <FieldGroup className="space-y-5">
                  <div className="flex flex-col items-center gap-2 text-center">
                    <p className="text-muted-foreground text-sm">
                      Une session a été trouvée pour{" "}
                      <strong>{username || session?.username}</strong>. Veuillez entrer votre mot de
