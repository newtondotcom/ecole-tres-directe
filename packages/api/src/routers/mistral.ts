...
function createNameTransformer(studentFirstName: string, placeholderName: string): NameTransformer {
  ...
  return {
    anonymize: (value: string) => replaceWithMatchedCase(value, studentPattern, placeholderName),
    deanonymize: (value: string) =>
      replaceWithMatchedCase(value, placeholderPattern, trimmedStudentName),
  };
}

function replaceWithMatchedCase(value: string, pattern: RegExp, replacement: string) {
  if (!value) {
    return value;
  }

  pattern.lastIndex = 0;
  return value.replace(pattern, (match) => applyCasePattern(match, replacement));
}
...
function buildUserPrompt(
  prompt: string,
  subjects: z.infer<typeof subjectAppreciationSchema>[],
  ...
  const subjectsSummary = subjects
    .map((subject) => {
      const lines = [
        ...
        subject.appreciation ? `  Appréciation: ${subject.appreciation}` : "  Appréciation: (vide)",
      ].filter(Boolean);
      return lines.join("\n");
    })
    .join("\n");
  ...
  generateAppreciation: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      if (!input.subjects.length) {
        throw new Error("Impossible de générer l'appréciation sans données de matières.");
      }

      const placeholderName = input.studentGender === "F" ? "Marie" : "Pierre";
      const nameTransformer = createNameTransformer(input.studentFirstName, placeholderName);
      ...
      if (!content) {
        throw new Error("La réponse de Mistral ne contient pas de contenu exploitable.");
      }
